...
def get_init_fn(sess):
    ...
    if FLAGS.restore_pretrain:
        ...
        ck_global_step =0 if FLAGS.restore_path[-1]=='t' else int(FLAGS.restore_path.split('-')[-1])
        variables_to_restore = get_variables_to_restore()
        restorer = tf.train.Saver(variables_to_restore)
        restorer.restore(sess, FLAGS.restore_path)
        print("load the model successfully,whose path is %s!!!"%FLAGS.restore_path)
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            if FLAGS.restore_scopes=='stage2':
                variables_to_restore = get_variables_to_restore()
            else:
...
def get_variables_to_restore():
    ...
    if FLAGS.restore_scopes=='stage2':
        scopes=[FLAGS.model_name]
        if FLAGS.IMAGE_PART_LOSS==True:
            scopes.append('build_conv_features')
            scopes.append('build_image_part_cls')
        if FLAGS.IMAGE_ID_LOSS==True:
            scopes.append('image_ID_loss')
            scopes.append('joint_embedding')
        if FLAGS.TEXT_ID_LOSS==True:
            # scopes.append('text_ID_loss')
            scopes.append('BiLSTM')
    if FLAGS.restore_scopes=='stage3':
        scopes=[FLAGS.model_name]
        if FLAGS.CMPC==True:
            scopes.append('Wfc')
        if FLAGS.IMAGE_PART_LOSS==True:
